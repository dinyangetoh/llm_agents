...
def search_semantic_scholar(query: str, limit: int = 8) -> List[dict]:
    ...
    """
    key = os.getenv("SEMANTIC_SCHOLAR_API_KEY", "").strip()
    allow_anon = os.getenv("SEMANTIC_SCHOLAR_ALLOW_ANONYMOUS", "").lower() in ("1", "true", "yes")
    ...
    params = {"query": query[:2000], "limit": limit, "fields": SEMANTIC_SCHOLAR_FIELDS}
    ...
    for attempt in range(5):
        try:
            ...
            if response.status_code == 429:
                ...
                print(f"[Semantic Scholar] 429 — backing off {wait:.1f}s (attempt {attempt + 1}/5)")
                time.sleep(wait)
                continue
        ...
        for attempt in range(4):
            try:
                ...
                if response.status_code in (429, 500, 502, 503, 504):
                    ...
                    print(f"[CORE] HTTP {response.status_code} — retry in {wait:.1f}s")
                    time.sleep(wait)
                    continue
                response.raise_for_status()
                data = response.json()
